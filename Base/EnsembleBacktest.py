...
class EnsembleBacktest(object):
    ...
    def __init__(self, Xs, y, base_learners, meta_model, start_date, meta_start, end_date, window='expanding', window_size=120, num_stocks=None, classif = False):
        self.Xs = Xs
    ...
    def make_base_forecasts(self):
        ...
        for mod in self.base_learners:
            #handle models that need individual models per stock
            if mod.__class__ == AutoArima:
                self.model = []
            ...
            elif mod.__class__ == NeuralNetwork:
                self.model = []
                i = 0
                while i < self.y.shape[1]:
                    self.model.append(mod.build_model(self.Xs[0].shape[1], 1))
                    i += 1
            else:
                self.model = mod
            ...
            preds = pd.DataFrame(all_pred_returns, columns=cols, index=index)
            self.all_base_preds.append(preds)
        return self

    def make_meta_dfs(self):
        self.make_base_forecasts()
        index = self.y.index[(self.y.index >= self.start_date) & (
                self.y.index <= self.end_date)]  # list of dates we will be iterating through
        cols = self.y.columns
        #loop through the base_learners preds and bind the assets together
        feats_df = np.zeros((len(self.all_base_preds), self.all_base_preds[0].shape[0]))
        self.all_meta_dfs = [feats_df for i in range(self.y.shape[1])]
        for i in range(self.y.shape[1]):
            for j in range(len(self.all_base_preds)):
                self.all_meta_dfs[i][j] = self.all_base_preds[j].iloc[:,i]
            self.all_meta_dfs[i] = pd.DataFrame(np.transpose(self.all_meta_dfs[i]), index=index)

        return self

    def meta_month_forecast(self, date):
        #this line below should only be used once which will be in the meta_all_forecasts
        # self.make_meta_dfs()
        preds = []
        for i in range(len(self.all_meta_dfs)):
            #x_train, x_test
            x = self.all_meta_dfs[i]
            x_train = x[x.index < date]
            x_test = x[x.index == date]
            #y_train
            y_train = self.y.iloc[:,i]
            #make y start where the base_learner preds start
            y_train = y_train[(y_train.index >= self.start_date) & (y_train.index < date)]
            mod= self.meta_model.fit(x_train, y_train)
            pred = mod.predict(x_test)
            preds.append(pred.item())

        return preds

    def all_meta_forecasts(self):
        self.make_meta_dfs()
        index = self.y.index[(self.y.index >= self.meta_start) & (
                self.y.index <= self.end_date)]  # list of dates we will be iterating through
        cols = self.y.columns  # list of column name for when we create the dateframe at the end
        ...
        for forecast_date in index:
            forecast_date = str(forecast_date.year) + '-' + str(forecast_date.month) + '-' + str(forecast_date.day)
            month_preds = self.meta_month_forecast(forecast_date)
            all_meta_preds.append(month_preds)

        self.preds = pd.DataFrame(all_meta_preds, index=index, columns=self.actual.columns)
        return self
