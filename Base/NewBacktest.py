...
class Backtest(object):
    ...
    def __init__(self, Xs, y, model, start_date, end_date, window='expanding', window_size=120, num_stocks=None, classif = False, coefs=False):
        self.Xs = Xs
        ...
        self.coefs = coefs #if you want to save the coefs or not
        if num_stocks == None:
    ...
    def month_forecast(self, date):
        ...
        for i in range(len(self.Xs)):
            ...
            if self.is_NN:
                ...
                if self.coefs == True:
                    coefs = mod.coef_.ravel().tolist()

            pred = mod.predict(x_test)
            forecasts.append(pred.item())

        return forecasts, coefs
    ...
    def all_forecasts(self):
        ...
        for forecast_date in index:
            ...
            month_returns, month_coefs = self.month_forecast(forecast_date)
            all_pred_returns.append(month_returns)
            all_coefs.append(month_coefs)

        self.preds = pd.DataFrame(all_pred_returns, columns=cols, index=index)
        if self.coefs == True:
            pd.DataFrame(all_coefs, columns=)
        return self
    ...
    def sMAPE(self):
        ...
        smape = (2/self.actual.shape[0])*np.sum(np.sum(val))*100
        return smape
...
test_mod1 = NewBacktest(all_xs, y, LinearRegression(), '2000-01', '2002-12', coefs=True).month_forecast('2001-02-28')
